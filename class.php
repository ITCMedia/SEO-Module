...
class Updater{
	...
	public function isUpdate($deepLevel, $informationsystem_id, $title, $description, $keywords){
		...
		if($informationsystem_group_id != ''){
			...
			$result = mysql_query ("UPDATE `informationsystem_groups` SET `seo_title`='{$title}', `seo_description`='{$description}', `seo_keywords`='{$keywords}'  WHERE `path` ='{$deepLevel}' AND `informationsystem_id` ='{$informationsystem_id}' AND `deleted` ='0'");
			if ($result == 'true')
			{
