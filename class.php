...
class Updater{
	...
	public function shopUpdate($deepLevel, $shop_id, $title, $description, $keywords){
		...
		if($shop_group_id != ''){
			...
			$result = mysql_query ("UPDATE `shop_groups` SET `seo_title`='{$title}', `seo_description`='{$description}', `seo_keywords`='{$keywords}'  WHERE `path` ='{$deepLevel}' AND `shop_id` ='{$shop_id}' AND `deleted` ='0'");
			if ($result == 'true')
			{
	...
	public function isUpdate($deepLevel, $informationsystem_id, $title, $description, $keywords){
		...
		if($informationsystem_group_id != ''){
			...
			$result = mysql_query ("UPDATE `informationsystem_groups` SET `seo_title`='{$title}', `seo_description`='{$description}', `seo_keywords`='{$keywords}'  WHERE `path` ='{$deepLevel}' AND `informationsystem_id` ='{$informationsystem_id}' AND `deleted` ='0' AND `deleted` ='0'");
			if ($result == 'true')
			{
